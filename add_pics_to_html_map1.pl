...
#	#for testing
	...
	say $folder;
	say "______";
   my @pictures = map { s/"$pictures_dir\/$folder\/"//, $_ } glob("$pictures_dir/$folder/*" );
   
   #for testing. Note that all the ones in @pictures will get a link for the non-existent thumbnail.
   # If you use quotes inside the map block, this wont happen, but worse things will happen.
	say "-----";
   say "@pictures";
   say "-----";
...
# need to get out of $folder, or remove_tree (below) will complain.
#chdir ("$work_dir");
...
foreach my $folder (@thumbnail_folders) {
	my @thumbnails = map { s/$thumbnails_dir\/$folder\///, $_ } glob("$thumbnails_dir/$folder/*" );

# This line is for testing. Ignore the ones in the output.
	say "@thumbnails";
